...
def user_product_freq(user_id = None):
	...
	products = pd.read_csv('./data/order_products__prior.csv')

	orders = orders[orders.user_id == user_id];
	if len(orders) == 0 : return -1;
	products = products[products.order_id.isin(orders.order_id)];

	orders.drop(['user_id','eval_set','order_number','days_since_prior_order'],axis=1,
		inplace=np.bool_(True))
	products.drop(['add_to_cart_order','reordered'],axis=1,inplace=np.bool_(True));
	...
	def set_freq(product, computed_hour):
		...
		print product.product_id, computed_hour
		#pass
