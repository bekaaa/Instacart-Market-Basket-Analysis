...
def predict_s(size = 0,rand = False, eval_set='test'):
	...
	if rand :
		# There is a Bug here, remeber to fix it :)
	...
	outputfile = './data/output/predictions/predictions-%s-%d-%s.csv' % ( eval_set, size, rand )
...
def predict_t():
	...
	outputfile = './data/output/predictions/predictions-test-%s.csv' % str(np.datetime64('now'))
